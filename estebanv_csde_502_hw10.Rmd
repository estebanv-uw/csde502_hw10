...
#Step 0.0.7: Loading data
  ...
  df.pop = read.table(file = url.pop, header = TRUE)
  df.dth = read.table(file = url.dth, header = TRUE)
  names(df.pop) %<>% tolower()
  names(df.dth) %<>% tolower()
  
  ##Subsetting data to last 50 years
  df.pop %<>% filter(year >= 1970) %>% select(year, age, total)
  df.dth %<>% filter(year >= 1970) %>% select(year, age, total)
  names(df.pop)[3] = "n"
  names(df.dth)[3] = "d"
  ...
  merge(by = "year",
        x = df,
        y = aggregate(formula = cbind(ptotal = n) ~ year, 
                      data = df, 
                      FUN = sum))
  
  ##Summing death totals by year
  df = 
  merge(by = "year",
        x = df,
